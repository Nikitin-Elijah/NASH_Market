...
from src.auth import verify_password, create_access_token, create_refresh_token, hash_password, get_current_user
from src.config import SECRET_KEY, ALGORITHM, s3_storage, STORAGE_URL
from src.models.users import UserModel
from src.schemas.users import UserSchema, UserCreate
...
async def create_user(user_data: UserCreate):
    users = await UserModel.filter(email=user_data.email)
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    """
    result = await UserModel.filter(email=form_data.username)
    ...
    access_token = create_access_token(data={"sub": user.email, "username": user.username, "id": user.id})
    refresh_token = create_refresh_token(data={"sub": user.email, "username": user.username, "id": user.id})
...
async def refresh_token(refresh_token: str):
    ...
    result = await UserModel.filter(email=email)
    user = result[0] if result else None
