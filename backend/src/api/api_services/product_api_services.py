...
class AddProductAPIService:
    ...
    def __init__(
        self, add_product_service: AddProductService = Depends()
    ):
...
class GetUserProductsAPIService:
    ...
    async def _get_user_products(self, user_id: int) -> list[ProductModel]:
        products = (
            await self.session.scalars(
                select(ProductModel)
                .where(ProductModel.seller_id == user_id, ProductModel.is_active == True)
    ...
    async def exec(self, user_id: int) -> list[ProductModel]:
        return await self._get_user_products(user_id=user_id)
...
class GetProductAPIService:
    ...
    def __init__(
        self, get_product_service: GetProductService = Depends()
    ):
...
class PaginatedProductsAPIService:
    ...
    async def _paginated_products(self, limit: int, offset: int) -> PaginatedResponse:
        db_products = (
            await self.session.scalars(
                select(ProductModel)
        ...
        items = (
            db_products[offset : offset + limit]
            if offset + limit < total_count
            else db_products[offset:]
        )
    ...
    @redis_cache(prefix='products', ttl=1200)
    async def exec(self, limit: int, offset: int) -> PaginatedResponse:
        return await self._paginated_products(limit=limit, offset=offset)
