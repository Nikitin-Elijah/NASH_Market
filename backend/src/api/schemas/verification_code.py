...
class ProtectVerificationCodeSchema(BaseModel):
    id: int = Field(description="ID попытки регистрации пользователя")
    user_id: int = Field(description="ID пользователя")
    code: str = Field(description="Код для подтверждения")
    activate: bool = Field(description="Флаг - был ли использован код")
